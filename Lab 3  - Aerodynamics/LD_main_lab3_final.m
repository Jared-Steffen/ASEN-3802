...
a_0_0006_rad = a_0_0006 * 180/pi;
a_0_0012_rad = a_0_0012 * 180/pi;
a_0_0018_rad = a_0_0018 * 180/pi;
a_0_2412_rad = a_0_2412 * 180/pi;
a_0_4412_rad = a_0_4412 * 180/pi;

% alpha_L0_0006_2 = -p_cl0006_2(2) / p_cl0006_2(1);
% alpha_L0_0012 = -p_cl0012(2) / p_cl0012(1);
% alpha_L0_0018 = -p_cl0018(2) / p_cl0018(1);           % Alternate method
% alpha_L0_2412 = -p_cl2412(2) / p_cl2412(1);           % Uses polyfit
% alpha_L0_4412 = -p_cl4412(2) / p_cl4412(1);

alpha_L0_0006_2_rad = alpha_L0_0006_2 * pi/180;
alpha_L0_0012_rad = alpha_L0_0012 * pi/180;
alpha_L0_0018_rad = alpha_L0_0018 * pi/180;
alpha_L0_2412_rad = alpha_L0_2412 * pi/180;
alpha_L0_4412_rad = alpha_L0_4412 * pi/180;
...
a0_r5 = a_0_2412_rad;        % a0 root
a0_t5 = a_0_0012_rad;        % a0 tip
aero_r5 = alpha_L0_2412_rad; % a_L0 root
aero_t5 = alpha_L0_0012_rad; % a_L0 tip
S5 = 0.5 * (ct5 + cr5) * b5;
rho5 = 17.56E-4;
mu5 = 3.534E-7;
...
cL_SS = 0.513066830173821; % c_L steady state = 0.513066830173821
cDi_SS = 0.011671104290211; % c_Di steady state = 0.011671104290211
...
c_d_0012 = 0.006;         % Taken from experimental data at Reynold's number = 3*10^6 (similar to our Re5 calculated number)
c_d_2412 = 0.0065;

c_d_avg = (c_d_0012 + c_d_2412)/2;

C_D = c_d_avg + cDi_tolerance_PLLT4;        % Calculating cd with estimated values and our lowest error cDi
D = 0.5*rho5*V5^2*C_D*S5;

LoD = L_3/D;        % For lowest error L/D problem 5

geor_vec = linspace(-10,20,50);
geor_vec_rad = geor_vec .* pi/180;
geot_vec = geor_vec - 1;
geot_vec_rad = geot_vec .* pi/180;

for i=1:length(geor_vec_rad)        % Finding L/D at varying angle of attack

[~,c_L2,c_Di2] = PLLT(b5,a0_t5,a0_r5,ct5,cr5,aero_t5,aero_r5,geot_vec_rad(i),geor_vec_rad(i),20);


% cd vector
% NACA data
cd0012 = [0.0136 0.0054 0.0136];
cl0012 = [-1 0 1];

cd2412 = [0.008 0.006 0.0091];
cl2412 =  [ -0.65 0.2 1.08];

% quadratic fit
fit0012 = polyfit(cl0012,cd0012,2);
fit2412 = polyfit(cl2412,cd2412,2);

% average root and tip c_d to get c_d vec
c_d2 = (polyval(fit0012 ,c_L2)+polyval(fit2412 ,c_L2))/2;
...
fprintf('Problem 2: \n')
fprintf('\n')
fprintf("NACA 0006 lift slope in 1/rad = %d \n",a_0_0006_rad)
fprintf("NACA 0006 zero lift AOA in degrees = %d \n",alpha_L0_0006_2)
fprintf('\n')
fprintf("NACA 0012 lift slope in 1/rad = %d \n",a_0_0012_rad)
fprintf("NACA 0012 zero lift AOA in degrees = %d \n",alpha_L0_0012)
fprintf('\n')
fprintf("NACA 0018 lift slope in 1/rad = %d \n",a_0_0018_rad)
fprintf("NACA 0018 zero lift AOA in degrees = %d \n",alpha_L0_0018)
fprintf('\n')
fprintf("Symmetric TAT lift slope in 1/rad = %d \n",a_0_TAT*(180/pi))
fprintf("Symmetric TAT zero lift AOA in degrees = %d \n",0)
fprintf('\n')
...
fprintf("NACA 0012 lift slope in 1/rad = %d \n",a_0_0012_rad)
fprintf("NACA 0012 zero lift AOA in degrees = %d \n",alpha_L0_0012)
fprintf("TAT NACA 0012 lift slope in 1/rad = %d \n",a_0_TAT*(180/pi))
fprintf("TAT NACA 0012 zero lift AOA in degrees = %d \n",0)
fprintf('\n')
fprintf("NACA 2412 lift slope in 1/degrees = %d \n",a_0_2412_rad)
fprintf("NACA 2412 zero lift AOA in degrees = %d \n",alpha_L0_2412)
fprintf("TAT NACA 2412 lift slope in 1/degrees = %d \n",a_0_2412_TAT*(180/pi))
fprintf("TAT NACA 2412 zero lift AOA in degrees = %d \n",alpha_L0_TAT_2412)
fprintf('\n')
fprintf("NACA 4412 lift slope in 1/rad = %d \n",a_0_4412_rad)
fprintf("NACA 4412 zero lift AOA in degrees = %d \n",alpha_L0_4412)
fprintf("TAT NACA 4412 lift slope in 1/rad = %d \n",a_0_4412_TAT*(180/pi))
fprintf("TAT NACA 4412 zero lift AOA in degrees = %d \n",alpha_L0_TAT_4412)
fprintf('\n')

% Problem 5
fprintf('Problem 5: \n')
fprintf('\n')
fprintf("Steady state lift in lb_f = %d \n", 0.5*rho5*V5^2*S5*cL_SS)
...
fprintf('\n')
fprintf("Steady state induced drag in lb_f = %d \n", 0.5*rho5*V5^2*S5*cDi_SS)
...
plot(N3,Di5)
grid on
ylabel('Induced Drag [lb_f]')
xlabel('Number of Terms')
title('Induced Drag Convergence')

figure(7)
plot(geot_vec,LoD_vec)
grid on
ylabel('Lift over Drag (Aerodynamic Efficiency)')
xlabel('Angle of Attack (degrees)')
title('Aerodynamic Efficiency vs. AoA')
